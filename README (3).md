...
`bridge_consumer.py` runs on your laptop and loops:
...
| 4 — Bronze | 10 | Explicit schema; timestamps distinguished; checkpoint-vs-consumer-group question answered |
...
**Marks are lost for:** committed tokens or `.env`; screenshots without explanation; claims asserted but not demonstrated (especially Task 5.6); `overwrite=True` on batch uploads; unbounded loops in notebooks; absolute paths that only work on your machine.
